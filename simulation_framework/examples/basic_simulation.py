...
def main():
    ...
    try:
        ...
        print(f"Configuration: {config.world_width}x{config.world_height} world, max {config.max_ticks} ticks")

        # Create simulation
        simulation = Simulation(config)
        simulation.initialize_simulation("Basic Example", "A simple demonstration of the simulation framework")
        ...
        for i, agent in enumerate(agents):
            if i % 2 == 0:
                ...
                agent.current_goals = [GatherResourceGoal(resource_type="wood", target_quantity=5)]
        ...
        print(f"Simulation completed after {simulation.time_manager.current_tick} ticks")
        ...
        if report:
            ...
            if 'population_metrics' in report:
                ...
                print(f"Population Growth Rate: {pop_metrics.get('growth_rate', 0):.2%}")
                print(f"Population Density: {pop_metrics.get('density', 0):.2f}")

        # Show agent status
        print("\n=== Agent Status ===")
        for agent in simulation.agents:
            print(f"{agent.name}: Health {agent.stats.health}/{agent.stats.max_health}, "
