...
class AnalyticsEngine:
    ...
    def _calculate_economic_metrics(
        ...
        if world_snapshots:
            ...
            if market_prices:
                ...
                self._save_metric(
                    simulation_id, current_tick, "max_item_price", max_price, "economy"
                )

        # Trade volume and frequency
        recent_trades = self.db.get_action_logs(
            simulation_id, action_type="trade", start_tick=max(0, current_tick - 50)
        )

        trade_volume = len(recent_trades)
        self._save_metric(
            simulation_id, current_tick, "recent_trade_volume", trade_volume, "economy"
        )
    ...
    def _calculate_social_metrics(self, simulation_id: int, current_tick: int) -> None:
        ...
        recent_agents = self.db.get_agent_snapshots(
            simulation_id, start_tick=max(0, current_tick - 10), end_tick=current_tick
        )
        ...
        for snapshot in recent_agents:
            if (
                snapshot.agent_id not in agent_data
                or snapshot.tick > agent_data[snapshot.agent_id].tick
            ):
        ...
        agent_positions = [
            (agent.position_x, agent.position_y) for agent in agent_data.values()
        ]
        clustering_metric = self._calculate_clustering(agent_positions)
        self._save_metric(
            simulation_id, current_tick, "agent_clustering", clustering_metric, "social"
        )
    ...
    def _calculate_combat_metrics(self, simulation_id: int, current_tick: int) -> None:
        ...
        recent_combats = self.db.get_action_logs(
            simulation_id, action_type="combat", start_tick=max(0, current_tick - 100)
        )

        combat_frequency = len(recent_combats) / 100.0  # per tick
        self._save_metric(
            simulation_id, current_tick, "combat_frequency", combat_frequency, "combat"
        )
        ...
        death_actions = self.db.get_action_logs(
            simulation_id, action_type="death", start_tick=max(0, current_tick - 100)
        )

        death_rate = len(death_actions) / 100.0  # per tick
        self._save_metric(
            simulation_id, current_tick, "death_rate", death_rate, "combat"
        )

    def _calculate_exploration_metrics(
        self, simulation_id: int, current_tick: int
    ) -> None:
        ...
        explore_actions = self.db.get_action_logs(
            simulation_id, action_type="explore", start_tick=max(0, current_tick - 50)
        )
        ...
        recent_agents = self.db.get_agent_snapshots(
            simulation_id, start_tick=current_tick, end_tick=current_tick
        )

        if recent_agents:
            positions = [
                (agent.position_x, agent.position_y) for agent in recent_agents
            ]
    ...
    def _calculate_agent_performance_metrics(
        ...
        self._save_metric(
            simulation_id, current_tick, "average_agent_health", avg_health, "agents"
        )
        ...
        self._save_metric(
            simulation_id, current_tick, "average_agent_stamina", avg_stamina, "agents"
        )
        ...
        self._save_metric(
            simulation_id, current_tick, "average_agent_gold", avg_gold, "economy"
        )
    ...
    def get_trend_analysis(
        self, simulation_id: int, metric_name: str, window_size: int = 100
    ) -> Dict[str, Any]:
        """Analyze trends for a specific metric"""
        analytics_data = self.db.get_analytics(
            simulation_id, metric_name=metric_name, limit=window_size
        )
    ...
    def get_correlation_analysis(
        self, simulation_id: int, metric1: str, metric2: str
    ) -> Dict[str, Any]:
        ...
        try:
            correlation = (
                statistics.correlation(values1, values2) if len(values1) > 1 else 0
            )
    ...
    def generate_simulation_report(self, simulation_id: int) -> Dict[str, Any]:
        ...
        for metric in key_metrics:
            latest_data = self.db.get_analytics(
                simulation_id, metric_name=metric, limit=1
            )
            if latest_data:
                report["key_metrics"][metric] = latest_data[0].metric_value
                report["trends"][metric] = self.get_trend_analysis(
                    simulation_id, metric, window_size=50
                )
