...
from visual_simulation import find_valid_spawn_positions, create_diverse_agents, create_varied_npcs


def main():
    """Run a visual simulation with persistent database"""
    parser = argparse.ArgumentParser(description="MMO Simulator with Persistent Database")
    parser.add_argument('--width', type=int, default=25, help='World width (default: 25)')
    parser.add_argument('--height', type=int, default=25, help='World height (default: 25)')
    parser.add_argument('--agents', type=int, default=6, help='Number of agents (default: 6)')
    parser.add_argument('--npcs', type=int, default=4, help='Number of NPCs (default: 4)')
    ...
    parser.add_argument('--db-file', type=str, default='simulation_data.db', help='Database file path')
    ...
    try:
        ...
        try:
            ...
            if recent_actions:
                ...
                for tick, agent_id, action_type, success, message in recent_actions:
                    ...
                    print(f"  Tick {tick}: Agent {agent_id} {action_type} {status} - {message}")
        ...
        print(f"   cursor.execute('SELECT * FROM action_logs LIMIT 10')")
        print(f"   print(cursor.fetchall())")
