...
def test_threat_detection():
    ...
    assert len(threats) > 0, f"Agent should detect at least one threat, found {len(threats)}"
...
def test_flee_goal_generation():
    ...
    assert len(flee_goals) > 0, f"Should generate at least one FleeFromDangerGoal, got goals: {[type(g).__name__ for g in potential_goals]}"

    flee_goal = flee_goals[0]
    assert flee_goal.danger_source_id == goblin.id, f"Flee goal should target goblin {goblin.id}, got {flee_goal.danger_source_id}"
...
def test_priority_based_interruption():
    ...
    if initial_action:
        print(f"✓ Agent had initial action: {type(initial_action).__name__} from goal priority {initial_goal.priority if initial_goal else None}")
        ...
        if new_action != initial_action or (new_goal and new_goal.priority > (initial_goal.priority if initial_goal else 0)):
            ...
            print("✓ Action was not interrupted (may be expected if action was not interruptible)")
    else:
        print("✓ No initial action to interrupt, but system is ready for interruptions")
...
def test_complete_interrupt_scenario():
    ...
    agent.personality = Personality(
        bravery=0.2, caution=0.8, aggression=0.1, curiosity=0.6,
        industriousness=0.5, greed=0.3, patience=0.5, sociability=0.5
    )
    ...
    initial_distance = agent.distance_to(goblin)
    ...
    for step in range(10):
        ...
        print(f"Step {step}: Agent {old_pos}->{new_pos}, Goals {old_goals}->{new_goals}, Distance: {distance_to_goblin:.1f}")
        ...
        if distance_to_goblin > goblin.aggro_range and not agent_escaped:
            print(f"  ★ Agent escaped! Distance {distance_to_goblin:.1f} > aggro range {goblin.aggro_range}")
            agent_escaped = True
            break
