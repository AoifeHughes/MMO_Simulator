...
class ForestFisherCooperationScenario(BaseScenario):
    ...
    async def setup(self, server):
        ...
        logger.info(
            f"Scenario using custom small {self.map_size}x{self.map_size} map for close cooperation"
        )
    ...
    def _create_custom_terrain(self, world_map):
            ...
            for x in range(width):
                ...
                if left_forest:
                    ...
                    if (x + y) % 3 == 0:
                        world_map.tiles[y][
                            x
                        ] = TileType.WOOD  # WOOD tiles for harvesting
                    else:
        ...
        logger.info(
            f"  Forest area: Left third with concentrated forest near wood cutter"
        )
    ...
    async def spawn_agents(self) -> List[Dict[str, Any]]:
        ...
        woodcutter_id = self.server.world.spawn_agent(
            "explorer", woodcutter_x, woodcutter_y, 45.0
        )
        woodcutter_state = self.server.agent_registry.register_agent(
            woodcutter_id, "explorer", woodcutter_x, woodcutter_y
        )
        ...
        if woodcutter_state:
            ...
            if hatchet and not any(
                hasattr(item, "tool_type") and item.tool_type == "woodcutting"
        ...
        logger.info(
            f"Spawned Wood Cutter {woodcutter_id} at ({woodcutter_x}, {woodcutter_y})"
        )
        ...
        fisher_state = self.server.agent_registry.register_agent(
            fisher_id, "explorer", fisher_x, fisher_y
        )
        ...
        logger.info(
            f"  Map Size: {self.map_size}x{self.map_size} for close cooperation"
        )
        logger.info(
            f"  Wood Cutter: Harvests wood → Crafts fires (basic_fire recipe: 2 wood)"
        )
        ...
        logger.info(
            f"  Distance between agents: ~{((fisher_x - woodcutter_x)**2 + (fisher_y - woodcutter_y)**2)**0.5:.1f} units"
        )
