...
class PathfindingTestScenario(BaseScenario):
    def __init__(self):
        super().__init__(
            name="Pathfinding Test",
            description="Test scenario for pathfinding with predetermined waypoints"
        )
        self.test_waypoints = [
            (10, 10),  # Start point
    ...
    async def spawn_agents(self) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Spawned pathfinding test agent {agent_id} at ({start_x}, {start_y})")
