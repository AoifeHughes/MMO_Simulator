...
class Camera:
    ...
    def world_to_screen(
        self, world_x: int, world_y: int, tile_size: int
    ) -> Tuple[int, int]:
...
class GameVisualizer:
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    if simulation.agents:
                        ...
                        self.camera.x = (
                            agent.position[0] * self.tile_size - self.width // 2
                        )
                        self.camera.y = (
                            agent.position[1] * self.tile_size - self.height // 2
                        )
    ...
    def _handle_agent_click(self, mouse_pos: Tuple[int, int], simulation: Simulation):
        ...
        world_x, world_y = self.camera.screen_to_world(
            mouse_pos[0], mouse_pos[1], self.tile_size
        )
    ...
    def render(self, simulation: Simulation):
        ...
        visible_bounds = self._get_visible_bounds(
            simulation.world.width, simulation.world.height
        )
    ...
    def _render_terrain(
        self, simulation: Simulation, visible_bounds: Tuple[int, int, int, int]
    ):
        ...
        for y in range(min_y, max_y):
            for x in range(min_x, max_x):
                if 0 <= x < simulation.world.width and 0 <= y < simulation.world.height:
                    ...
                    if tile:
                        color = self.TERRAIN_COLORS.get(
                            tile.terrain_type, (100, 100, 100)
                        )
    ...
    def _render_agents(
        ...
        for agent in simulation.agents:
            ...
            if min_x <= agent_x < max_x and min_y <= agent_y < max_y:
                screen_x, screen_y = self.camera.world_to_screen(
                    agent_x, agent_y, self.tile_size
                )
                ...
                if (
                    ...
                    and agent.stats.health < agent.stats.max_health
                ):
        ...
        for npc in simulation.npcs:
            ...
            if min_x <= npc_x < max_x and min_y <= npc_y < max_y:
                ...
                pygame.draw.rect(
                    ...
                    (
                        screen_x
                        + (self.tile_size * self.camera.zoom - scaled_size) // 2,
                        screen_y
                        + (self.tile_size * self.camera.zoom - scaled_size) // 2,
                        scaled_size,
                        scaled_size,
                ...
                if self.camera.zoom >= 1.5 and npc.stats.health < npc.stats.max_health:
                    self._draw_health_bar(
                        ...
                        scaled_size,
                        npc.stats.health,
                        npc.stats.max_health,
                    )

    def _draw_health_bar(
        self, x: int, y: int, width: int, current_health: int, max_health: int
    ):
        """Draw a health bar"""
        bar_height = 4
        ...
        # Foreground (green)
        pygame.draw.rect(
            self.screen, (0, 255, 0), (x, y, int(width * health_ratio), bar_height)
        )
