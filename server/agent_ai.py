...
class ServerAI:
    ...
    def register_agent(self, agent_id: str, agent_type: str, x: float, y: float):
        ...
        if agent_id not in self.agent_states:
            self.agent_states[agent_id] = AIAgentState(
                agent_id=agent_id, patrol_center=(x, y), current_patrol_target=(x, y)
            )
            logger.info(
                f"[SERVER AI] Registered agent {agent_id[:8]} ({agent_type}) for AI control"
            )
    ...
    def unregister_agent(self, agent_id: str):
        ...
        if agent_id in self.agent_states:
            ...
            logger.info(
                f"[SERVER AI] Unregistered agent {agent_id[:8]} from AI control"
            )
    ...
    def update_ai_tick(
        self,
        world_state: Dict[str, AgentData],
        ...
        for agent_id, agent_data in world_state.items():
            ...
            if agent_id not in self.agent_states:
                self.register_agent(
                    agent_id, agent_data.agent_type, agent_data.x, agent_data.y
                )
    ...
    def _process_agent_ai(
        self,
        agent: AgentData,
        ai_state: AIAgentState,
        behavior: BehaviorProfile,
        visible_entities: List[AgentData],
        current_time: float,
    ) -> List[Dict[str, Any]]:
        ...
        enemies_in_sight = self._find_enemies_in_range(
            agent, visible_entities, behavior
        )
        nearest_enemy = (
            self._find_nearest_enemy(agent, enemies_in_sight)
            if enemies_in_sight
            else None
        )

        # Determine new AI state based on current situation
        new_state = self._determine_ai_state(
            agent, ai_state, behavior, nearest_enemy, current_time
        )

        # Change state if needed
        if new_state != ai_state.current_state and self._can_change_state(
            ai_state, behavior, current_time
        ):
            logger.debug(
                f"[SERVER AI] Agent {agent.id[:8]} state change: {ai_state.current_state} -> {new_state}"
            )
        ...
        state_actions = self._execute_ai_state(
            agent, ai_state, behavior, nearest_enemy, current_time
        )
    ...
    def _find_enemies_in_range(
        ...
        for entity in visible_entities:
            if (
                entity.agent_type in behavior.enemy_types
                and entity.is_alive
    ...
    def _find_nearest_enemy(
        self, agent: AgentData, enemies: List[AgentData]
    ) -> Optional[AgentData]:
    ...
    def _determine_ai_state(
        self,
        agent: AgentData,
        ai_state: AIAgentState,
        behavior: BehaviorProfile,
        nearest_enemy: Optional[AgentData],
        current_time: float,
    ) -> AIState:
        ...
        if (
            agent.health <= behavior.flee_health_threshold
            and current_time - agent.last_damage_time < 3.0
        ):
            return AIState.FLEE
    ...
    def _can_change_state(
        self, ai_state: AIAgentState, behavior: BehaviorProfile, current_time: float
    ) -> bool:
        """Check if enough time has passed to allow state change"""
        return (
            current_time - ai_state.last_state_change >= behavior.state_change_cooldown
        )

    def _execute_ai_state(
        self,
        agent: AgentData,
        ai_state: AIAgentState,
        behavior: BehaviorProfile,
        ...
        if ai_state.current_state == AIState.COMBAT:
            actions.extend(
                self._execute_combat(agent, ai_state, behavior, nearest_enemy)
            )

        elif ai_state.current_state == AIState.CHASE:
            actions.extend(
                self._execute_chase(agent, ai_state, behavior, nearest_enemy)
            )
            ...
            actions.extend(
                self._execute_patrol(agent, ai_state, behavior, current_time)
            )
    ...
    def _execute_combat(
        ...
        if enemy and self._calculate_distance(agent, enemy) <= behavior.attack_range:
            ...
            logger.debug(
                f"[SERVER AI] Agent {agent.id[:8]} attacking {enemy.id[:8]} with {attack_name}"
            )
    ...
    def _execute_chase(
        ...
        if enemy:
            ...
            if distance > 0:
                ...
                logger.debug(
                    f"[SERVER AI] Agent {agent.id[:8]} chasing {enemy.id[:8]} at distance {distance:.1f}"
                )
    ...
    def _execute_flee(
        ...
        if distance > 0:
            ...
            logger.debug(
                f"[SERVER AI] Agent {agent.id[:8]} fleeing with {agent.health:.1f} health"
            )
    ...
    def _execute_patrol(
        ...
        if (
            ai_state.current_patrol_target is None
            or current_time - ai_state.last_patrol_change
            >= behavior.patrol_change_interval
            or self._calculate_distance_to_point(agent, ai_state.current_patrol_target)
            < 1.0
        ):
    ...
    def _calculate_distance_to_point(
        self, agent: AgentData, point: Tuple[float, float]
    ) -> float:
    ...
    def _get_attack_for_agent_type(self, agent_type: str) -> str:
        ...
        attack_mapping = {"player": "sword_slash", "enemy": "claw", "npc": "punch"}
