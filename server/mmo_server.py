...
class ClientConnection:
        ...
        try:
            while self.connected:
                ...
                try:
                    ...
                    logger.error(
                        f"Error handling message from client {self.client_id}: {e}"
                    )
    ...
    async def handle_connect(self, message: Message):
        ...
        if not agent_id:
            await self.send_message(
                Message(
                    ...
                    {"success": False, "message": "Failed to spawn agent"},
                )
            )
            return
        ...
        agent_state = self.server.agent_registry.register_agent(
            agent_id, agent_type, spawn_x, spawn_y
        )
        ...
        self.server.inventory_manager.get_or_create_inventory(
            agent_id, agent_state.inventory
        )
        ...
        logger.info(
            f"Client {self.client_id} connected as agent {agent_id} ({agent_type}) at ({spawn_x}, {spawn_y})"
        )
    ...
    async def handle_action(self, message: Message):
        ...
        try:
            ...
            await self.send_message(
                Message(MessageType.ACTION_RESPONSE, response.to_dict())
            )
...
class MMOGameServer:
    ...
    async def sync_positions_with_legacy(self):
        ...
        for entity_id in list(self.game_state.entities.keys()):
            pos_component = self.game_state.get_component(
            ...
            if pos_component:
                ...
                if agent_state:
                    agent_state.update_position(
                        pos_component.x, pos_component.y, pos_component.rotation
                    )
        ...
        while self.running:
            try:
                ...
                for channel in UpdateChannel:
                    entities_to_update = self.game_state.get_entities_by_channel(
                        channel
                    )
                    ...
                    if entities_to_update:
                        ...
                        for client in self.clients.values():
                            if (
                                client.connected
                                and client.agent_id
                                and client.should_send_update(channel, current_time)
                            ):
                                await self.send_partial_world_state(
                                    client, entities_to_update
                                )
                ...
                await asyncio.sleep(
                    1.0 / 120.0
                )  # 120Hz update check (faster than any send frequency)

            except Exception as e:
                logger.error(f"Error in client update loop: {e}")
                await asyncio.sleep(1.0)

    async def send_partial_world_state(
        self, client: ClientConnection, entities: Dict[str, Dict[str, Any]]
    ):
        ...
        try:
            ...
            logger.error(
                f"Error sending partial state to client {client.client_id}: {e}"
            )
        ...
        while self.running:
            try:
                ...
                for agent_id, client_id in list(self.client_agents.items()):
                    health_component = self.game_state.get_component(
                        agent_id, "HealthComponent"
                    )
                    if health_component and not health_component.is_alive:
                        ...
                        if agent_state and agent_state.respawn_time <= current_time:
                            ...
                            logger.info(
                                f"Respawned agent {agent_id} at ({spawn_x}, {spawn_y})"
                            )
    ...
    def get_world_state_for_client(self, agent_id: str) -> Dict[str, Any]:
        ...
        world_state = {"agents": [], "world_objects": [], "timestamp": time.time()}
        ...
        if hasattr(self.world, "world_objects"):
            world_state["world_objects"] = [
                obj.to_dict() for obj in self.world.world_objects.get_all_objects()
            ]
