...
class InventorySnapshot(Base):
    ...
    id = Column(Integer, primary_key=True)
    agent_snapshot_id = Column(
        Integer, ForeignKey("agent_snapshots.id"), nullable=False
    )
...
class DatabaseManager:
    ...
    async def initialize(self):
        ...
        self.engine = create_async_engine(
            f"sqlite+aiosqlite:///{self.database_path}", echo=False
        )
    ...
    async def start_scenario(
        self, scenario_name: str, world_width: int, world_height: int
    ) -> int:
        ...
        async with self.session_factory() as session:
            ...
            logger.info(
                f"Started scenario '{scenario_name}' with ID {self.current_scenario_id}"
            )
        ...
        async with self.session_factory() as session:
            ...
            for agent_id, agent_state in agent_registry.agents.items():
                await self._save_agent_snapshot(
                    session, agent_id, agent_state, snapshot_time
                )
    ...
    async def record_trade(
        ...
        agent1_id: str,
        agent2_id: str,
        agent1_items: List[Dict],
        agent2_items: List[Dict],
    ...
    async def record_craft(
        ...
        async with self.session_factory() as session:
            ...
            await session.commit()
            logger.debug(
                f"Recorded craft by {agent_id}: {recipe_name} -> {result_item}"
            )
    ...
    async def get_scenario_summary(
        ...
        async with self.session_factory() as session:
            ...
            return {
                ...
                "end_time": scenario_run.end_time.isoformat()
                if scenario_run.end_time
                else None,
...
class PeriodicDataCollector:
    ...
    def __init__(
        self, database_manager: DatabaseManager, agent_registry: AgentRegistry
    ):
