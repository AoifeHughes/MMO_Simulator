...
class TestEnemyAgents:
    ...
    async def test_enemy_spawning(self, game_server, agent_clients):
        ...
        assert len(enemy_agents) >= 2, f"Expected at least 2 enemies, found {len(enemy_agents)}"

    @pytest.mark.timeout(15)
    async def test_enemy_basic_movement(self, game_server, agent_clients, agent_tracker):
    ...
    async def test_enemy_patrol_behavior(self, game_server, agent_clients, agent_tracker):
        ...
        EnemyAssertions.assert_patrol_behavior(agent_tracker, enemy.agent_id, expected_patrol_area=15.0)

        agent_tracker.print_debug_info()

    @pytest.mark.timeout(25)
    async def test_enemy_player_detection(self, game_server, agent_clients, behavior_metrics):
        ...
        while time.time() - start_time < 20:
            ...
            if enemy.agent:
                ...
                behavior_metrics.record_agent_position(enemy.agent_id, "enemy", pos, current_time)

            if player.agent:
                pos = (player.agent.x, player.agent.y)
                behavior_metrics.record_agent_position(player.agent_id, "player", pos, current_time)

            # Check for chase behavior
            if enemy.agent and player.agent:
                distance = math.sqrt(
                    (enemy.agent.x - player.agent.x)**2 + (enemy.agent.y - player.agent.y)**2
                )

                if distance < 15.0:  # Within chase range
                    # Record state transition to chase
                    behavior_metrics.record_state_transition(enemy.agent_id, "chase", current_time)
    ...
    async def test_enemy_attack_behavior(self, game_server, agent_clients, behavior_metrics):
        ...
        while time.time() - start_time < 25:
            ...
            if enemy.agent and player.agent:
                ...
                behavior_metrics.record_agent_position(enemy.agent_id, "enemy", enemy_pos, current_time)
                behavior_metrics.record_agent_position(player.agent_id, "player", player_pos, current_time)
                ...
                if distance <= 2.5:  # Attack range
                    behavior_metrics.record_state_transition(enemy.agent_id, "attack", current_time)
                    behavior_metrics.record_interaction(
                    ...
                    behavior_metrics.record_state_transition(enemy.agent_id, "patrol", current_time)
        ...
        attack_interactions = [e for e in behavior_metrics.interaction_events if e['type'] == 'attack']
        chase_interactions = [e for e in behavior_metrics.interaction_events if e['type'] == 'chase']
        print(f"Interactions: {len(attack_interactions)} attacks, {len(chase_interactions)} chases")

    @pytest.mark.timeout(20)
    async def test_enemy_state_transitions(self, game_server, agent_clients, behavior_metrics):
        ...
        while time.time() - start_time < 18:
            ...
            if enemy.agent and player.agent:
                ...
                behavior_metrics.record_state_transition(enemy.agent_id, expected_state, current_time)
                transition_count += 1
        ...
        AgentAssertions.assert_state_transitions(
            behavior_metrics, enemy.agent_id,
            expected_states=["patrol", "chase"], min_transitions=3
        )
    ...
    @pytest.mark.timeout(25)
    async def test_multiple_enemies_behavior(self, game_server, agent_clients, agent_tracker, behavior_metrics):
        ...
        for enemy in enemies:
            if enemy.agent:
                AgentAssertions.assert_agent_moved(agent_tracker, enemy.agent_id, min_distance=2.0)

        # Check for interactions
        total_interactions = len(behavior_metrics.interaction_events)
        assert total_interactions > 0, "No interactions recorded between enemies and player"

        agent_tracker.print_debug_info()

    @pytest.mark.timeout(35)
    async def test_enemy_aggression_levels(self, game_server, agent_clients, behavior_metrics):
        ...
        for agent_id, behavior in enemy_behaviors.items():
            ...
            aggression_score = (behavior['chase_time'] + behavior['attack_time'] * 2) / len(behavior['distances'])
            aggression_scores.append(aggression_score)

            print(f"  Enemy {agent_id[:8]}: avg_dist={avg_distance:.1f}, aggression={aggression_score:.2f}")
        ...
        if len(aggression_scores) >= 2:
            ...
            assert aggression_variance > 0.1, f"Not enough aggression variation: {aggression_variance:.3f}"

    @pytest.mark.slow
    @pytest.mark.timeout(60)
    async def test_enemy_long_term_behavior(self, game_server, agent_clients, agent_tracker, behavior_metrics):
        ...
        for enemy in enemies:
            if enemy.agent:
                AgentAssertions.assert_agent_moved(agent_tracker, enemy.agent_id, min_distance=15.0)

        # Should have seen multiple behavior types
        assert enemy_analysis['chase_events'] > 0, "No chase events in long-term test"

        # Performance should remain acceptable
        AgentAssertions.assert_performance_acceptable(behavior_metrics, min_tick_rate=25.0)

    async def test_enemy_performance_under_load(self, game_server, agent_clients, behavior_metrics):
        ...
        while time.time() - start_time < 15:
            ...
            # Record performance
            active_count = len([e for e in enemies if e.connected]) + (1 if player and player.connected else 0)
            behavior_metrics.record_performance(
                tick_rate=30.0,
        ...
        print(f"Performance with {actual_count} enemies: {avg_sample_time*1000:.1f}ms avg sample time")

        assert avg_sample_time < 0.15, f"Performance degraded: {avg_sample_time*1000:.1f}ms > 150ms"
        AgentAssertions.assert_performance_acceptable(behavior_metrics, min_tick_rate=20.0)