...
class ActionTestBase:
    ...
    def mock_server(self):
        ...
        server.world.server = (
            server  # Set up the circular reference that ActionProcessor expects
        )
    ...
    def action_processor(self, mock_server):
        ...
        processor = ActionProcessor(
            mock_server.world, mock_server.agent_registry, attack_system
        )
    ...
    def create_harvest_request(
        self, agent_id: str, x: float = 5.0, y: float = 5.0
    ) -> ActionRequest:
    ...
    def create_craft_request(
        self, agent_id: str, recipe: str, x: float = 50.0, y: float = 50.0
    ) -> ActionRequest:
    ...
    def create_agent_with_wood(
        self, agent_id: str, wood_count: int = 5
    ) -> ServerAgentState:
        ...
        if wood_item:
            wood_item.name = (
                "wood"  # Force lowercase name to match crafting expectations
            )
    ...
    def setup_craft_mocks(
        self, mock_server, agent: ServerAgentState, fire_id: str = "fire_123"
    ):
    ...
    def assert_approved(self, response):
        ...
        assert (
            response.result == ActionResult.APPROVED
        ), f"Expected APPROVED but got {response.result}: {response.message}"

    def assert_rejected(self, response):
        """Assert that action was rejected using OOP assertion pattern"""
        assert (
            response.result == ActionResult.REJECTED
        ), f"Expected REJECTED but got {response.result}: {response.message}"
    ...
    def assert_error(self, response):
        ...
        ), f"Expected ERROR but got {response.result}: {response.message}"
