...
class TestNetworkFunctionality:
    ...
    async def test_client_server_connection(self, game_server, agent_clients):
        ...
        assert len(active_clients) >= 2, f"Expected at least 2 active clients, got {len(active_clients)}"

        # Verify server sees all clients
        server_agents = game_server.world.get_all_agents()
        assert len(server_agents) >= len(active_clients), "Server doesn't see all connected agents"
    ...
    async def test_message_synchronization(self, game_server, agent_clients):
        ...
        if client.agent:
            ...
            distance = ((server_pos[0] - new_x)**2 + (server_pos[1] - new_y)**2)**0.5

            # Allow some tolerance for movement interpolation
            assert distance < 2.0, f"Position not synchronized: server={server_pos}, expected=({new_x}, {new_y})"
    ...
    async def test_world_state_broadcast(self, game_server, agent_clients):
        ...
        for client in clients:
            ...
            assert world_state is not None, f"Client {client.agent_id[:8]} didn't receive world state"
            assert 'agents' in world_state, "World state missing agents data"

            agents_in_state = world_state['agents']
            assert len(agents_in_state) >= len(clients), f"Client sees {len(agents_in_state)} agents, expected {len(clients)}"
    ...
    async def test_client_disconnect_cleanup(self, game_server, agent_clients):
        ...
        if clients[0]:
            ...
            assert disconnected_agent_id not in remaining_ids, "Disconnected agent not cleaned up"
            assert len(remaining_agents) < initial_agent_count, "Agent count didn't decrease"

    async def test_high_frequency_updates(self, game_server, agent_clients, behavior_metrics):
        ...
        print(f"High frequency test: {update_count} updates in {test_duration:.1f}s ({updates_per_second:.1f} updates/sec)")

        assert updates_per_second >= 15.0, f"Update rate too low: {updates_per_second:.1f} < 15.0"
        ...
        if client.agent:
            ...
            if server_agent:
                ...
                server_pos = (server_agent.x, server_agent.y)
                sync_distance = ((client_pos[0] - server_pos[0])**2 + (client_pos[1] - server_pos[1])**2)**0.5

                assert sync_distance < 3.0, f"Poor synchronization after high-frequency updates: {sync_distance:.2f}"
    ...
    async def test_network_latency_simulation(self, game_server, agent_clients):
        ...
        if latency_samples:
            ...
            assert avg_latency < 0.2, f"Average latency too high: {avg_latency*1000:.1f}ms"
            assert max_latency < 0.5, f"Max latency too high: {max_latency*1000:.1f}ms"

    async def test_concurrent_client_updates(self, game_server, agent_clients, behavior_metrics):
        ...
        start_time = time.time()
        tasks = [asyncio.create_task(move_client(client, i)) for i, client in enumerate(clients)]
        ...
        for i, client in enumerate(clients):
            if client.agent:
                distance_moved = ((client.agent.x - 25)**2 + (client.agent.y - 25)**2)**0.5
                assert distance_moved > 5.0, f"Client {i} didn't move enough: {distance_moved:.2f}"
        ...
        assert sync_errors <= len(clients) * 0.3, f"Too many sync errors: {sync_errors}/{len(clients)}"
    ...
    async def test_network_recovery(self, game_server, agent_clients):
        ...
        if server_agent and client.agent:
            client_pos = (client.agent.x, client.agent.y)
            server_pos = (server_agent.x, server_agent.y)
            recovery_distance = ((client_pos[0] - server_pos[0])**2 + (client_pos[1] - server_pos[1])**2)**0.5

            print(f"Recovery test: client={client_pos}, server={server_pos}, distance={recovery_distance:.2f}")
            assert recovery_distance < 3.0, f"Failed to recover from network interruption: {recovery_distance:.2f}"

    async def test_bandwidth_efficiency(self, game_server, agent_clients, behavior_metrics):
        ...
        if "player" in message_counts and "npc" in message_counts:
            ...
            assert player_rate > npc_rate, f"Player rate ({player_rate:.1f}) should be higher than NPC rate ({npc_rate:.1f})"