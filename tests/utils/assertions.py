...
from tests.utils.agent_tracker import AgentTracker, AgentPath
...
class AgentAssertions:
    ...
    @staticmethod
    def assert_agent_moved(tracker: AgentTracker, agent_id: str, min_distance: float = 1.0):
    ...
    @staticmethod
    def assert_agent_stayed_in_area(tracker: AgentTracker, agent_id: str,
        ...
        for snapshot in path.snapshots:
            distance = math.sqrt((snapshot.x - center[0])**2 + (snapshot.y - center[1])**2)
            if distance > radius:
    ...
    @staticmethod
    def assert_agent_explored_area(tracker: AgentTracker, agent_id: str, min_tiles: int):
    ...
    @staticmethod
    def assert_exploration_efficiency(metrics: BehaviorMetrics, agent_id: str,
    ...
    @staticmethod
    def assert_movement_smoothness(metrics: BehaviorMetrics, agent_id: str,
    ...
    @staticmethod
    def assert_state_transitions(metrics: BehaviorMetrics, agent_id: str,
                               expected_states: List[str], min_transitions: int = 1):
        ...
        assert len(transitions) >= min_transitions, \
            f"Agent {agent_id} had only {len(transitions)} transitions, expected >= {min_transitions}"

    @staticmethod
    def assert_interaction_occurred(metrics: BehaviorMetrics, agent1_id: str, agent2_id: str,
                                   interaction_type: str):
        """Assert specific interaction occurred between agents"""
        interactions = [
            event for event in metrics.interaction_events
            if ((event['agent1'] == agent1_id and event['agent2'] == agent2_id) or
                (event['agent1'] == agent2_id and event['agent2'] == agent1_id)) and
               event['type'] == interaction_type
        ]
    ...
    @staticmethod
    def assert_performance_acceptable(metrics: BehaviorMetrics,
        ...
        assert avg_tick_rate >= min_tick_rate, \
...
class ExplorerAssertions:
    ...
    @staticmethod
    def assert_spiral_pattern(metrics: BehaviorMetrics, agent_id: str, tolerance: float = 2.0):
        ...
        if len(positions) < 10:
            raise AssertionError(f"Not enough position data for spiral analysis: {len(positions)}")
        ...
        assert max_distance > initial_distance + tolerance, \
            f"Spiral pattern not detected: max distance {max_distance:.2f} vs start {initial_distance:.2f}"

    @staticmethod
    def assert_random_exploration(metrics: BehaviorMetrics, agent_id: str,
                                min_directional_entropy: float = 0.7):
        ...
        if total == 0:
            raise AssertionError(f"Agent {agent_id} didn't move enough for directional analysis")
        ...
        assert normalized_entropy >= min_directional_entropy, \
            f"Agent {agent_id} movement too biased: entropy {normalized_entropy:.2f} < {min_directional_entropy}"

    @staticmethod
    def assert_no_overlap_with_other_explorers(tracker: AgentTracker, agent_id: str,
        ...
        other_explorers = [path for path in tracker.agent_paths.values()
        ...
        for snapshot in target_path.snapshots:
            for other_path in other_explorers:
                ...
                closest_snapshot = min(other_path.snapshots,
                                     key=lambda s: abs(s.timestamp - snapshot.timestamp),
                                     default=None)
                if closest_snapshot and abs(closest_snapshot.timestamp - snapshot.timestamp) < 1.0:
                    distance = math.sqrt((snapshot.x - closest_snapshot.x)**2 +
                                       (snapshot.y - closest_snapshot.y)**2)
                    if distance < min_separation:
                        violations.append((snapshot.timestamp, distance, other_path.agent_id))
...
class NPCAssertions:
    ...
    @staticmethod
    def assert_wandering_behavior(tracker: AgentTracker, agent_id: str,
                                max_wander_radius: float = 20.0):
        ...
        AgentAssertions.assert_agent_stayed_in_area(tracker, agent_id, home_pos, max_wander_radius)

    @staticmethod
    def assert_idle_wandering_cycle(metrics: BehaviorMetrics, agent_id: str,
        ...
        if idle_durations:
            ...
            assert expected_cycle_time[0] <= avg_idle <= expected_cycle_time[1], \
...
class EnemyAssertions:
    ...
    @staticmethod
    def assert_patrol_behavior(tracker: AgentTracker, agent_id: str,
                             expected_patrol_area: float = 15.0):
        """Assert enemy follows patrol pattern"""
        path = tracker.get_agent_path(agent_id)
        assert path is not None, f"No tracking data for agent {agent_id}"

        if len(path.snapshots) < 5:
            raise AssertionError(f"Not enough data for patrol analysis: {len(path.snapshots)}")
        ...
        returns_to_start = any(
            math.sqrt((pos[0] - start_center_x)**2 + (pos[1] - start_center_y)**2) < expected_patrol_area
            for pos in end_area
        )

        assert returns_to_start, \
    ...
    def assert_chase_behavior(metrics: BehaviorMetrics, agent_id: str):
        ...
        chase_interactions = [
            event for event in metrics.interaction_events
            if (event['agent1'] == agent_id or event['agent2'] == agent_id) and
               event['type'] == 'chase'
        ]

        assert len(chase_interactions) > 0, \
