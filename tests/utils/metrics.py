...
class PerformanceMetrics:
    ...
    def get_average_latency(self) -> float:
        return sum(self.network_latency) / len(self.network_latency) if self.network_latency else 0.0
...
class BehaviorMetrics:
    ...
    def __init__(self):
        ...
        self.performance: PerformanceMetrics = PerformanceMetrics(
            cpu_usage=[], memory_usage=[], tick_rates=[],
            network_latency=[], agent_counts=[], timestamps=[]
        )

    def record_agent_position(self, agent_id: str, agent_type: str,
                            position: Tuple[float, float], timestamp: float):
    ...
    def record_performance(self, tick_rate: float, agent_count: int,
                          timestamp: float, cpu_usage: float = 0.0,
    ...
    def analyze_explorer_behavior(self) -> Dict[str, Any]:
        ...
        explorer_patterns = {
            agent_id: pattern for agent_id, pattern in self.movement_patterns.items()
        ...
        if total_individual_tiles > 0:
            analysis['coverage_overlap'] = 1.0 - (total_unique_tiles / total_individual_tiles)
    ...
    def analyze_npc_behavior(self) -> Dict[str, Any]:
        ...
        for agent_id, pattern in npc_patterns.items():
            ...
            transition_frequency = len(transitions) / (pattern.timestamps[-1] - pattern.timestamps[0]) if len(pattern.timestamps) > 1 else 0
    ...
    def analyze_enemy_behavior(self) -> Dict[str, Any]:
        ...
        if patrol_efficiencies:
            analysis['patrol_efficiency'] = sum(patrol_efficiencies) / len(patrol_efficiencies)
