...
class TestDebugOutput:
    ...
    async def test_debug_data_collection(self, game_server, agent_clients, agent_tracker, behavior_metrics):
            ...
            for agent_client in agents:
                if agent_client.agent:
                    ...
                    debug_info = {
                        ...
                        'velocity': (getattr(agent_client.agent, 'velocity_x', 0),
                                   getattr(agent_client.agent, 'velocity_y', 0)),
                        'internal_state': getattr(agent_client.agent, 'behavior_state', 'unknown'),
                        'target': getattr(agent_client.agent, 'current_target', None),
                        'visible_entities': getattr(agent_client.agent, 'visible_entities', [])
                    }

                    # Add agent-specific debug data
                    if hasattr(agent_client.agent, 'explored_tiles'):
                        debug_info['explored_tiles'] = len(agent_client.agent.explored_tiles)
                    ...
                    if hasattr(agent_client.agent, 'home_x'):
                        debug_info['home_position'] = (agent_client.agent.home_x, agent_client.agent.home_y)
        ...
        assert len(debug_snapshots) >= 8, f"Expected at least 8 snapshots, got {len(debug_snapshots)}"
        ...
        print(f"Time span: {debug_snapshots[-1]['timestamp'] - debug_snapshots[0]['timestamp']:.1f}s")
    ...
    async def test_behavior_metrics_export(self, game_server, agent_clients, behavior_metrics):
            ...
            for agent in agents:
                if agent.agent:
                    ...
                    if agent.agent.agent_type == "enemy":
                        behavior_metrics.record_state_transition(agent.agent_id, "patrol", current_time)
                    elif agent.agent.agent_type == "npc":
                        state = "idle" if (int(current_time) % 6) < 3 else "wandering"
                        behavior_metrics.record_state_transition(agent.agent_id, state, current_time)
    ...
    async def test_agent_tracker_debug_output(self, game_server, agent_clients, agent_tracker):
    ...
    async def test_performance_monitoring(self, game_server, agent_clients, behavior_metrics):
        ...
        if performance_alerts:
            ...
            print(f"Worst performance: {worst_alert['tick_duration']*1000:.2f}ms with {worst_alert['agent_count']} agents")

        # Performance assertions
        assert avg_tick_time < 0.1, f"Average tick time too high: {avg_tick_time*1000:.1f}ms"
        assert len(performance_alerts) < len(tick_times) * 0.1, f"Too many performance alerts: {len(performance_alerts)}"

    async def test_debug_visualization_data(self, game_server, agent_clients, behavior_metrics):
        ...
        while time.time() - start_time < 12:
            ...
            for agent in agents:
                if agent.agent:
                    ...
                    visualization_data['agent_paths'][agent_id]['timestamps'].append(current_time)
        ...
        for agent_id, path_data in visualization_data['agent_paths'].items():
            ...
            if len(positions) >= 2:
                ...
                print(f"  Total distance: {total_distance:.2f}")
                print(f"  Start->End distance: {((end_pos[0]-start_pos[0])**2 + (end_pos[1]-start_pos[1])**2)**0.5:.2f}")

        # Analyze heatmap
        hottest_tile = max(visualization_data['heatmap_data'].items(), key=lambda x: x[1])
