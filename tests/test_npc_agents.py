...
class TestNPCAgents:
    ...
    async def test_npc_spawning(self, game_server, agent_clients):
        ...
        assert len(npc_agents) >= 2, f"Expected at least 2 NPCs, found {len(npc_agents)}"
    ...
    async def test_npc_basic_movement(self, game_server, agent_clients, agent_tracker):
        ...
        if initial_pos:
            NPCAssertions.assert_wandering_behavior(agent_tracker, agent_id, max_wander_radius=20.0)

        agent_tracker.print_debug_info()

    @pytest.mark.timeout(20)
    async def test_npc_idle_wander_cycle(self, game_server, agent_clients, behavior_metrics):
        ...
        while time.time() - start_time < 18:
            ...
            if npc.agent:
                ...
                behavior_metrics.record_agent_position(npc.agent_id, "npc", pos, current_time)

                # Estimate state based on movement
                if len(behavior_metrics.movement_patterns.get(npc.agent_id, {}).get('positions', [])) >= 2:
                    ...
                    if len(recent_positions) >= 2:
                        ...
                        if current_state != last_state:
                            behavior_metrics.record_state_transition(npc.agent_id, current_state, current_time)
        ...
        assert idle_count > 0, f"NPC never entered idle state (counts: idle={idle_count}, wander={wander_count})"
        assert wander_count > 0, f"NPC never entered wandering state (counts: idle={idle_count}, wander={wander_count})"

        print(f"State transitions: idle={idle_count}, wandering={wander_count}")

    @pytest.mark.timeout(25)
    async def test_npc_home_area_constraint(self, game_server, agent_clients, agent_tracker):
        ...
        for i, npc in enumerate(npcs):
            if npc.agent:
                ...
                distance_from_spawn = math.sqrt(
                    (current_pos[0] - spawn_pos[0])**2 + (current_pos[1] - spawn_pos[1])**2
                )
                assert distance_from_spawn <= 18.0, \
                    f"NPC {npc.agent_id[:8]} too far from spawn: {distance_from_spawn:.1f} > 18.0"

        agent_tracker.print_debug_info()

    @pytest.mark.timeout(18)
    async def test_npc_player_interaction(self, game_server, agent_clients, behavior_metrics):
        ...
        while time.time() - start_time < 15:
            ...
            if npc.agent and player.agent:
                distance = math.sqrt(
                    (npc.agent.x - player.agent.x)**2 + (npc.agent.y - player.agent.y)**2
                )
    ...
    @pytest.mark.timeout(20)
    async def test_multiple_npc_behavior(self, game_server, agent_clients, agent_tracker, behavior_metrics):
        ...
        for npc in npcs:
            if npc.agent:
                AgentAssertions.assert_agent_moved(agent_tracker, npc.agent_id, min_distance=1.0)

        # Check average wander radius is reasonable
        if 'average_wander_radius' in analysis and analysis['average_wander_radius'] > 0:
            assert analysis['average_wander_radius'] <= 20.0, \
                f"NPCs wandering too far: {analysis['average_wander_radius']:.1f}"

        agent_tracker.print_debug_info()

    @pytest.mark.timeout(30)
    async def test_npc_timing_behavior(self, game_server, agent_clients, behavior_metrics):
        ...
        if idle_durations:
            ...
            print(f"  Average idle time: {avg_idle:.1f}s (count: {len(idle_durations)})")
            assert 1.0 <= avg_idle <= 8.0, f"Idle time {avg_idle:.1f}s outside expected range 1-8s"

        if moving_durations:
            avg_moving = sum(moving_durations) / len(moving_durations)
            print(f"  Average moving time: {avg_moving:.1f}s (count: {len(moving_durations)})")
    ...
    async def test_npc_long_term_patterns(self, game_server, agent_clients, agent_tracker, behavior_metrics):
                ...
                for npc in npcs:
                    if npc.agent:
                        ...
                        behavior_metrics.record_performance(
                            tick_rate=30.0, agent_count=len(npcs), timestamp=current_time
                        )
        ...
        print(f"Average wander radius: {npc_analysis.get('average_wander_radius', 0):.2f}")
        ...
        for npc in npcs:
            if npc.agent:
                AgentAssertions.assert_agent_moved(agent_tracker, npc.agent_id, min_distance=5.0)

        assert npc_analysis.get('average_wander_radius', 0) <= 25.0, "NPCs wandering too far from home"

    async def test_npc_performance_under_load(self, game_server, agent_clients, behavior_metrics):
        ...
        print(f"Performance with {actual_count} NPCs: {avg_sample_time*1000:.1f}ms avg sample time")

        assert avg_sample_time < 0.1, f"Performance degraded: {avg_sample_time*1000:.1f}ms > 100ms"
