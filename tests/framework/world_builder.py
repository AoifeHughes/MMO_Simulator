...
class WorldBuilder:
    ...
    def _fill_area(self, start_x: int, start_y: int, width: int, height: int, tile_type: TileType):
    ...
    def add_corridor(self, start: Tuple[int, int], end: Tuple[int, int], width: int = 1) -> 'WorldBuilder':
    ...
    def add_room(self, top_left: Tuple[int, int], width: int, height: int,
                 door_positions: Optional[List[Tuple[int, int]]] = None) -> 'WorldBuilder':
    ...
    def add_island(self, center: Tuple[int, int], island_radius: int, water_radius: int) -> 'WorldBuilder':
        """Add island surrounded by water"""
        center_x, center_y = center

        # Add water
        for y in range(max(0, center_y - water_radius), min(self.height, center_y + water_radius + 1)):
            for x in range(max(0, center_x - water_radius), min(self.width, center_x + water_radius + 1)):
                distance = ((x - center_x) ** 2 + (y - center_y) ** 2) ** 0.5
                if distance <= water_radius:
                    self.terrain[(x, y)] = TileType.WATER

        # Add island in center
        for y in range(max(0, center_y - island_radius), min(self.height, center_y + island_radius + 1)):
            for x in range(max(0, center_x - island_radius), min(self.width, center_x + island_radius + 1)):
                distance = ((x - center_x) ** 2 + (y - center_y) ** 2) ** 0.5
    ...
    def add_random_obstacles(self, density: float = 0.1, obstacle_type: TileType = TileType.WALL) -> 'WorldBuilder':
        """Add random obstacles with specified density (0.0 to 1.0)"""
        for y in range(self.height):
            for x in range(self.width):
                if (x, y) not in [(s[1], s[2]) for s in self.agent_spawns]:  # Don't block spawn points
    ...
    def add_scattered_resources(self, resource_type: TileType, count: int) -> 'WorldBuilder':
        """Add randomly scattered resources"""
        walkable_tiles = [(x, y) for (x, y), tile in self.terrain.items()
