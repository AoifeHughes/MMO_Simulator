...
async def test_pathfinding_scenario_movement():
    ...
    try:
        ...
        test_agent = agents[0]
        assert test_agent.agent_type == "pathfinding_test", f"Expected pathfinding_test agent, got {test_agent.agent_type}"
        ...
        assert abs(initial_position[0] - expected_start[0]) < 10.0, f"Agent x position {initial_position[0]} not near expected {expected_start[0]}"
        assert abs(initial_position[1] - expected_start[1]) < 10.0, f"Agent y position {initial_position[1]} not near expected {expected_start[1]}"
        ...
        while time.time() - start_time < test_duration:
            ...
            if hasattr(client.agent, 'movement_state') and client.agent.movement_state == "completed":
                break

        # Get test results
        assert hasattr(client.agent, 'get_test_results'), "Agent doesn't have get_test_results method"
        ...
        assert len(visited_waypoints) >= 1, f"Agent visited {len(visited_waypoints)} waypoints, expected at least 1"

        # Verify movement occurred (position changed from start)
        final_position = results['current_position']
        distance_moved = ((final_position[0] - initial_position[0])**2 + (final_position[1] - initial_position[1])**2)**0.5
        assert distance_moved > 5.0, f"Agent only moved {distance_moved:.2f} units, expected significant movement"
        ...
        if visited_waypoints:
            print(f"  First waypoint reached: {visited_waypoints[0]['waypoint']} at time {visited_waypoints[0]['time_reached']:.2f}s")
...
async def test_pathfinding_scenario_waypoint_accuracy():
    ...
    try:
        ...
        while time.time() - start_time < test_duration:
            ...
            if hasattr(client.agent, 'visited_waypoints') and len(client.agent.visited_waypoints) > 0:
                break
        ...
        if len(visited_waypoints) > 0:
            ...
            distance_error = ((actual_position[0] - target_waypoint[0])**2 +
                             (actual_position[1] - target_waypoint[1])**2)**0.5

            assert distance_error <= 2.0, f"Agent reached waypoint with error of {distance_error:.2f}, expected <= 2.0"
