...
class TestWaterNavigation:
    ...
    def create_water_obstacle_map(self):
        ...
        terrain = MapBuilder(15, 15)\
            .add_rect(6, 6, 9, 9, TileType.WATER)\
            .build()
    ...
    def create_water_channel_map(self):
        ...
        terrain = MapBuilder(15, 15)\
            .add_rect(7, 2, 8, 13, TileType.WATER)\
            .build()
    ...
    def test_movement_around_water_obstacle(self):
        ...
        if success:
            ...
            assert self.world.validate_position(agent.x, agent.y), f"Agent at ({agent.x}, {agent.y}) should be on walkable terrain"

            # Agent should not have jumped too far from intended path
            distance_from_intended = math.sqrt((agent.x - 8.0)**2 + (agent.y - 7.0)**2)
            assert distance_from_intended < 3.0, f"Agent jumped too far from intended path: {distance_from_intended}"
            ...
            assert distance_from_start > 1.0, "Agent should have moved away from start position"
        else:
            # If movement was rejected, agent should be at original position
            assert abs(agent.x - 4.0) < 0.1 and abs(agent.y - 7.0) < 0.1, "Agent should remain at original position if movement rejected"
    ...
    def test_edge_movement_around_water(self):
        ...
        for target_x, target_y in movement_path:
            ...
            if success:
                assert self.world.validate_position(agent.x, agent.y), f"Agent should be on walkable terrain at ({agent.x}, {agent.y})"

                # Should be relatively close to intended position
                distance = math.sqrt((agent.x - target_x)**2 + (agent.y - target_y)**2)
                assert distance < 2.0, f"Agent too far from intended position: {distance}"
    ...
    def test_water_channel_navigation(self):
        ...
        if success:
            ...
            assert self.world.validate_position(agent.x, agent.y), "Agent should be on walkable terrain"
            ...
            assert agent.x < 7.0 or agent.x > 8.0, "Agent should not be in the water channel"
    ...
    def test_gradual_water_approach(self):
        ...
        for i, (x, y, success) in enumerate(positions):
            ...
            print(f"Step {i}: intended=({expected_x:.1f}, 7.0), actual=({x:.1f}, {y:.1f}), success={success}")
            ...
            if expected_x < 5.5:  # Before hitting water
                ...
                assert distance_to_intended < 0.5, f"Early movement should be close to intended position"

            # Agent should always be on walkable terrain
            assert self.world.validate_position(x, y), f"Agent should be on walkable terrain at step {i}"
    ...
    def test_position_precision_near_water(self):
        ...
        for start_x, start_y in test_positions:
            ...
            assert self.world.validate_position(agent.x, agent.y), f"Agent should be on valid terrain: ({agent.x}, {agent.y})"
            ...
            assert distance_to_target < 3.0, f"Agent should be reasonably close to target: distance={distance_to_target:.2f}"
    ...
    def test_no_infinite_correction_loops(self):
        ...
        if len(unique_positions) == 1:
            ...
            assert self.world.validate_position(stable_pos[0], stable_pos[1]), \
            ...
            assert max_movement < 8.0, f"Movement should not be too large: {max_movement:.2f}"

        # All positions should be valid
        for x, y in positions:
            assert self.world.validate_position(x, y), f"Position ({x}, {y}) should be valid"
...
class TestWaterTileProperties:
    ...
    def test_water_tile_consistency(self):
        ...
        assert water_props.movement_cost == float("inf"), "Water should have infinite movement cost"
    ...
    def test_world_map_water_walkability(self):
        ...
        # Test walkability
        assert not world.world_map.is_walkable(5, 5), "Water tile should not be walkable"
        assert world.world_map.get_movement_cost(5, 5) == float("inf"), "Water should have infinite movement cost"
