...
class TestMovementValidator:
    ...
    def test_validate_movement_distance_too_far(self):
        ...
        assert (
            validator.validate_movement_distance((0, 0), (8, 8)) is False
        )  # ~11.3 units
    ...
    def test_validate_movement_distance_edge_cases(self):
        ...
        assert (
            validator.validate_movement_distance((0, 0), (6, 8)) is True
        )  # Exactly 10
    ...
    def test_validate_movement_speed_valid(self):
        ...
        assert (
            validator.validate_movement_speed((0, 0), (2, 0), 1.0) is True
        )  # 2 units/sec
        assert (
            validator.validate_movement_speed((0, 0), (5, 0), 1.0) is True
        )  # 5 units/sec
        assert (
            validator.validate_movement_speed((0, 0), (3, 4), 1.0) is True
        )  # 5 units/sec
    ...
    def test_validate_movement_speed_too_fast(self):
        ...
        assert (
            validator.validate_movement_speed((0, 0), (10, 0), 1.0) is False
        )  # 10 units/sec
        assert (
            validator.validate_movement_speed((0, 0), (5, 0), 0.5) is False
        )  # 10 units/sec
    ...
    def test_validate_movement_speed_very_slow(self):
        ...
        assert (
            validator.validate_movement_speed((0, 0), (0.1, 0), 1.0) is True
        )  # 0.1 units/sec
    ...
    def test_validate_complete_movement_valid(self):
        ...
        result = validator.validate_complete_movement(
            ...
            target_pos=(
                13,
                14,
            ),  # ~5 units distance in 1 sec = 5 units/sec (within limit)
            delta_time=1.0,
        )
    ...
    def test_custom_validation_parameters(self):
        ...
        assert (
            validator.validate_movement_distance((0, 0), (15, 0)) is True
        )  # Was too far
        assert (
            validator.validate_movement_distance((0, 0), (0.1, 0)) is False
        )  # Now too small
        assert (
            validator.validate_movement_speed((0, 0), (8, 0), 1.0) is True
        )  # Was too fast
    ...
    def test_negative_coordinates(self):
        ...
        assert (
            validator.validate_movement_distance((-5, -5), (0, 0)) is True
        )  # ~7.07 units
        assert (
            validator.validate_movement_distance((-10, 0), (10, 0)) is False
        )  # 20 units
    ...
    def test_floating_point_precision(self):
        ...
        assert (
            validator.validate_movement_distance(
                (0.123456789, 0.987654321), (5.123456789, 4.987654321)
            )
            is True
        )
        ...
        assert (
            validator.validate_movement_distance((0.0, 0.0), (6.0, 8.0)) is True
        )  # Exactly 10.0
