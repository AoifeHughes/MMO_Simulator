...
class MockWorld:
    ...
    def move_agent(
        self, agent_id: str, x: float, y: float, rotation: float = 0, **kwargs
    ) -> bool:
    ...
    def get_visible_agents(
        self, agent_id: str, vision_range: float = 10.0
    ) -> List[AgentData]:
        ...
        for other_id, other_agent in self.agents.items():
            ...
            dist = (
                (other_agent.x - observer.x) ** 2 + (other_agent.y - observer.y) ** 2
            ) ** 0.5
...
class MockGameServer:
    ...
    async def process_agent_action(self, agent_id: str, action_data: Dict[str, Any]):
        ...
        self.sent_messages.append(
            {"type": "agent_action", "agent_id": agent_id, "data": action_data}
        )
...
class FastTestFixture:
    ...
    async def add_client(
        self, agent_type: str = "player", spawn_x: float = 10, spawn_y: float = 10
    ) -> MockClient:
