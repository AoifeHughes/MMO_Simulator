...
class TestMaps:
    ...
    @staticmethod
    def get_empty_arena(width: int = 20, height: int = 20) -> Dict[Tuple[int, int], TileType]:
    ...
    @staticmethod
    def get_combat_arena(width: int = 15, height: int = 15) -> Dict[Tuple[int, int], TileType]:
    ...
    @staticmethod
    def get_pathfinding_maze(width: int = 21, height: int = 21) -> Dict[Tuple[int, int], TileType]:
    ...
    @staticmethod
    def get_fishing_pond(width: int = 25, height: int = 25) -> Dict[Tuple[int, int], TileType]:
    ...
    @staticmethod
    def get_exploration_terrain(width: int = 30, height: int = 30) -> Dict[Tuple[int, int], TileType]:
        ...
        for y in range(height):
            for x in range(width):
                ...
                if x < 10:
                    ...
                    if y < 15:
                        terrain[(x, y)] = TileType.WATER if (x + y) % 4 == 0 else TileType.GRASS
                    else:
                        terrain[(x, y)] = TileType.WALL if (x * y) % 7 == 0 else TileType.GRASS

        return terrain

    @staticmethod
    def get_corridor_test(width: int = 50, height: int = 10) -> Dict[Tuple[int, int], TileType]:
    ...
    @staticmethod
    def get_multi_room_dungeon(width: int = 40, height: int = 30) -> Dict[Tuple[int, int], TileType]:
...
class MapBuilder:
    ...
    def add_rect(self, x1: int, y1: int, x2: int, y2: int, tile_type: TileType) -> "TestMapBuilder":
    ...
    def add_circle(self, center_x: int, center_y: int, radius: float, tile_type: TileType) -> "TestMapBuilder":
    ...
    def add_line(self, x1: int, y1: int, x2: int, y2: int, tile_type: TileType) -> "TestMapBuilder":
...
def create_test_world_map(terrain_dict: Dict[Tuple[int, int], TileType], width: int, height: int):
