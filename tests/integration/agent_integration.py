...
class AgentIntegrationTester:
    ...
    def run_integration_test_suite(
        self, suite_name: str = "comprehensive"
        ...
        summary = self._generate_suite_summary(
            passed, failed, skipped, errors, total_tests
        )
        ...
        # Save results
        suite.save_to_file(
            str(self.output_dir / f"{suite_name}_integration_results.json")
        )
        return suite
    ...
    def _run_behavior_interrupt_integration(self):
        ...
        def test_func():
            ...
            return {
                ...
                "details": {"interrupt_system_accessible": interrupt_handled},
            }
    ...
    def _run_flexibility_regression_integration(self):
        ...
        def test_func():
            ...
            integration_success = (
                flexibility_report.overall_flexibility > 0.0
                and regression_report.overall_score >= 0.0
            )
    ...
    def _run_agent_lifecycle_integration(self):
        ...
        def test_func():
            ...
            lifecycle_stages.append(
                "composition_created" if composition1 else "composition_failed"
            )
            ...
            lifecycle_stages.append(
                "memory_formed" if memory_formed else "memory_failed"
            )
            ...
            lifecycle_stages.append(
                "adaptation_success" if adaptation_success else "adaptation_failed"
            )
            ...
            lifecycle_stages.append(
                "interrupt_handled" if interrupt_success else "interrupt_failed"
            )
            ...
            lifecycle_stages.append(
                "performance_measured" if performance_measured else "performance_failed"
            )
            ...
            success_count = len(
                [
                    ...
                    or "formed" in stage
                    or "handled" in stage
                    or "measured" in stage
                ]
            )
            overall_success = (
                success_count >= 4
            )  # At least 4 out of 5 stages successful
    ...
    def _run_multi_agent_interaction_integration(self):
        ...
        def test_func():
            ...
            agent1 = MockFlexibilityAgent(
                "social_agent", Personality(combat=3.0, exploration=5.0, social=9.0)
            )
            agent2 = MockFlexibilityAgent(
                "combat_agent", Personality(combat=9.0, exploration=4.0, social=3.0)
            )

            interactions_successful = 0

            # Test social memory formation
            agent1.memory.remember_social_interaction(
                "combat_agent", "cooperation", "successful"
            )
            agent2.memory.remember_social_interaction(
                "social_agent", "cooperation", "successful"
            )
            ...
            social_context = {
                "partner_agent": "combat_agent",
                "interaction_type": "cooperation",
            }
            composition1 = self.behavior_composer.compose_behavior(
                agent1, social_context
            )

            combat_context = {
                "partner_agent": "social_agent",
                "interaction_type": "cooperation",
            }
            composition2 = self.behavior_composer.compose_behavior(
                agent2, combat_context
            )
            ...
            multi_agent_context = {
                "multi_agent": True,
                "agents": ["social_agent", "combat_agent"],
            }
            flexibility_report = self.flexibility_harness.run_flexibility_assessment(
            ...
            success = (
                interactions_successful >= 4
            )  # At least 4 out of 5 interactions successful

            return {
                "success": success,
                "performance": {
                    "interaction_success_rate": interactions_successful / 5.0,
                    "social_memory_formation": len(
                        agent1.memory.social_memory.agent_memories
                    ),
    ...
    def _run_performance_integration(self):
        ...
        def test_func():
            ...
            performance_metrics["avg_composition_time"] = sum(composition_times) / len(
                composition_times
            )
            ...
            performance_metrics["avg_memory_operation_time"] = sum(memory_ops) / len(
                memory_ops
            )
            ...
            start = time.time()
            self.flexibility_harness.run_flexibility_assessment(
                agent, ["resource_scarcity_adaptation"]
            )
            ...
            composition_acceptable = (
                performance_metrics["avg_composition_time"] < 0.1
            )  # < 100ms
            memory_acceptable = (
                performance_metrics["avg_memory_operation_time"] < 0.01
            )  # < 10ms
            flexibility_acceptable = (
                performance_metrics["flexibility_assessment_time"] < 5.0
            )  # < 5s

            overall_performance = (
                composition_acceptable and memory_acceptable and flexibility_acceptable
            )
            ...
            return {
                ...
                "details": {
                    "composition_performance": "acceptable"
                    if composition_acceptable
                    else "slow",
                    "memory_performance": "acceptable" if memory_acceptable else "slow",
                    "flexibility_performance": "acceptable"
                    if flexibility_acceptable
                    else "slow",
    ...
    def _run_stress_test_integration(self):
        ...
        def test_func():
            ...
            for agent in agents:
                for _ in range(10):  # 10 rapid compositions per agent
                    try:
                        ...
                        composition = self.behavior_composer.compose_behavior(
                            agent, context
                        )
            ...
            stress_results["composition_failure_rate"] = composition_failures / (
                len(agents) * 10
            )
                ...
                for i in range(20):  # 20 memory operations per agent
                    try:
                        agent.memory.remember_resource_location(
                            i, i, f"stress_resource_{i}", 0.5, 1
                        )
                    except Exception:
                        memory_failures += 1
            ...
            composition_success = (
                stress_results["composition_failure_rate"] < 0.1
            )  # < 10% failure
            memory_success = (
                stress_results["memory_failure_rate"] < 0.05
            )  # < 5% failure
            flexibility_success = (
                stress_results["flexibility_failure_rate"] < 0.2
            )  # < 20% failure

            overall_success = (
                composition_success and memory_success and flexibility_success
            )
            ...
            return {
                ...
                "details": {
                    ...
                    "composition_reliability": "good"
                    if composition_success
                    else "poor",
                    "memory_reliability": "good" if memory_success else "poor",
                    "flexibility_reliability": "good"
                    if flexibility_success
                    else "poor",
                },
            }
    ...
    def _run_complete_scenario_integration(self):
        ...
        def test_func():
            ...
            agent = MockFlexibilityAgent(
                "e2e_agent", Personality(combat=6.0, exploration=7.0, social=5.0)
            )
            scenario_steps = []
            ...
            scenario_steps.append(
                "resource_discovery"
                if resource_memory_formed
                else "resource_discovery_failed"
            )

            # Step 3: Agent encounters danger and adapts
            agent.memory.remember_danger_zone(
                26.0, 31.0, "hostile_creature", 0.8, {"threat_type": "creature"}
            )
            danger_context = {"threat_detected": True, "location": (26.0, 31.0)}
            danger_composition = self.behavior_composer.compose_behavior(
                agent, danger_context
            )
            scenario_steps.append(
                "danger_adaptation"
                if danger_composition
                else "danger_adaptation_failed"
            )

            # Step 4: Agent meets another agent and forms social memory
            agent.memory.remember_social_interaction(
                "friendly_trader", "trade", "successful", details={"items_traded": 5}
            )
            social_memory_formed = len(agent.memory.social_memory.agent_memories) > 0
            scenario_steps.append(
                "social_interaction"
                if social_memory_formed
                else "social_interaction_failed"
            )
            ...
            try:
                ...
                interrupt_handled = False
            scenario_steps.append(
                "emergency_handled" if interrupt_handled else "emergency_failed"
            )
            ...
            scenario_steps.append(
                "flexibility_assessed"
                if flexibility_success
                else "flexibility_assessment_failed"
            )
            ...
            baseline_created = regression_tester.create_baseline(
                "e2e_scenario", "1.0.0", [agent_config], {}
            )
            if baseline_created:
                regression_report = regression_tester.test_regression(
                    "e2e_scenario", "1.0.0", agent_config, {}
            ...
            scenario_steps.append(
                "regression_check" if regression_success else "regression_check_failed"
            )

            # Calculate scenario success
            successful_steps = len(
                [step for step in scenario_steps if not step.endswith("_failed")]
            )
            scenario_success = (
                successful_steps >= 6
            )  # At least 6 out of 7 steps successful
            ...
            return {
                ...
                "details": {
                    ...
                    "social_connections": len(
                        agent.memory.social_memory.agent_memories
                    ),
                },
            }
    ...
    def _generate_suite_summary(
        self, passed: int, failed: int, skipped: int, errors: int, total: int
    ) -> str:
