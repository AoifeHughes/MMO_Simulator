...
def test_flee_goal_generation():
    ...
    ), f"Flee goal should target goblin {goblin.id}"
    assert (
...
def test_priority_based_interruption():
    ...
    goal_prio = initial_goal.priority
    print(f"✓ Agent started {action_name} from goal priority {goal_prio}")
...
def test_complete_interrupt_scenario():
    ...
    for step in range(8):
        ...
        old_action = (
            type(agent.current_action).__name__ if agent.current_action else None
        )
        ...
        print(
            ...
            f"Action {old_action}->{new_action}, "
            f"Goals {old_goals}->{new_goals}, "
            f"Distance to goblin: {distance_to_goblin:.1f}"
        )
        ...
        if flee_goals:
            print(
                f"  -> Agent has FleeFromDangerGoal (priority {flee_goals[0].priority})!"
            )

        # Check if we're outside aggro range
        if distance_to_goblin > goblin.aggro_range:
            aggro = goblin.aggro_range
            print(f"  -> Agent escaped! Outside aggro range ({aggro})")
            break
