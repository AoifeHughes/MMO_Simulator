...
class PositionPredictor:
    ...
    @staticmethod
    def get_action_position(
        agent_x: float,
        agent_y: float,
        target_x: float,
        target_y: float,
        max_approach_distance: float = 0.5,
    ) -> Tuple[float, float]:
        """
...
class PositionSyncManager:
    ...
    def get_current_position(self, agent_id: str) -> Optional[Tuple[float, float]]:
        ...
        if dt < self.max_prediction_time and (
            state.velocity_x != 0 or state.velocity_y != 0
        ):
    ...
    def validate_action_position(
        ...
        suggested_distance = (
            (target_x - suggested_pos[0]) ** 2 + (target_y - suggested_pos[1]) ** 2
        ) ** 0.5

        if suggested_distance <= max_distance:
            return (
                False,
                f"{action_name} distance {distance:.2f} > {max_distance:.2f}, can move to better position",
                suggested_pos,
            )
        else:
            return (
                False,
                f"{action_name} distance {distance:.2f} > {max_distance:.2f}, target unreachable",
                None,
            )

    def smooth_position_correction(
        self,
        agent_id: str,
        server_x: float,
        server_y: float,
        ...
        if correction_distance <= max_correction:
            ...
            logger.info(
                f"Smooth position correction for {agent_id[:8]}: "
        ...
        logger.warning(
            f"Large position correction for {agent_id[:8]}: "
...
def update_agent_position(
    ...
    global_position_sync.update_agent_position(
        agent_id, x, y, velocity_x, velocity_y, is_predicted
    )
...
def validate_action_position(
    ...
    return global_position_sync.validate_action_position(
        agent_id, target_x, target_y, max_distance, action_name
    )


def smooth_position_correction(
    agent_id: str, server_x: float, server_y: float, max_correction: float = 2.0
) -> Tuple[float, float]:
    """Convenience function for smooth position correction"""
    return global_position_sync.smooth_position_correction(
        agent_id, server_x, server_y, max_correction
    )
