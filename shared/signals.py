...
def create_help_request(sender_id: str, sender_pos: tuple,
                       enemy_count: int = 0, help_type: str = "general") -> Signal:
...
def create_resource_found(sender_id: str, sender_pos: tuple,
                         resource_type: str, resource_location: tuple,
                         quantity: int = 1) -> Signal:
...
def create_enemy_spotted(sender_id: str, sender_pos: tuple,
                        enemy_location: tuple, enemy_count: int = 1,
                        enemy_types: List[str] = None) -> Signal:
...
def create_trade_offer(sender_id: str, sender_pos: tuple,
                      offering: List[Dict[str, Any]],
                      requesting: List[Dict[str, Any]],
                      target_agent: Optional[str] = None) -> Signal:
...
def create_area_clear(sender_id: str, sender_pos: tuple,
                     cleared_area: tuple) -> Signal:
    ...
    return Signal(
        ...
        data={
            "cleared_area": cleared_area,  # (x, y, radius)
            "safety_level": "high"
        },
        priority=SignalPriority.NORMAL,
        sender_position=sender_pos,
...
def create_gather_signal(sender_id: str, sender_pos: tuple,
                        gather_purpose: str = "general",
                        max_agents: int = 5) -> Signal:
...
def create_danger_warning(sender_id: str, sender_pos: tuple,
                         danger_area: tuple, danger_type: str = "unknown") -> Signal:
...
def create_retreat_signal(sender_id: str, sender_pos: tuple,
                         retreat_to: Optional[tuple] = None) -> Signal:
...
def create_follow_me_signal(sender_id: str, sender_pos: tuple,
                           destination: Optional[tuple] = None,
                           purpose: str = "unknown") -> Signal:
...
def create_task_complete_signal(sender_id: str, sender_pos: tuple,
                               task_type: str, task_location: Optional[tuple] = None) -> Signal:
...
class SignalFilter:
    ...
    def _adjust_preference_for_personality(self, signal: Signal,
                                         base_preference: float, personality) -> float:
        ...
        if hasattr(personality, 'social'):
            ...
            if signal.signal_type in [SignalType.GATHER_HERE, SignalType.FOLLOW_ME,
        ...
        if hasattr(personality, 'combat'):
            ...
            if signal.signal_type in [SignalType.ENEMY_SPOTTED, SignalType.RETREAT,
