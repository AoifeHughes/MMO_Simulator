...
class PositionStatsCollector:
    ...
    def __init__(self):
        ...
        self.action_stats: Dict[str, ActionDistanceStats] = defaultdict(ActionDistanceStats)
    ...
    def record_position_discrepancy(
        ...
        if distance > DEBUG.MAX_ACCEPTABLE_DISCREPANCY:
            logger.warning(f"Large position discrepancy for {agent_id[:8]} during {action_context or 'unknown'}: "
    ...
    def record_action_distance_attempt(
        ...
        if action_name not in self.action_stats:
            self.action_stats[action_name] = ActionDistanceStats(action_name=action_name)

        stats = self.action_stats[action_name]

        # Update statistics
        stats.total_attempts += 1
        stats.max_distance_attempted = max(stats.max_distance_attempted, actual_distance)
        stats.min_distance_attempted = min(stats.min_distance_attempted, actual_distance)

        if was_successful:
            stats.successful_validations += 1
            # Update rolling average for successful attempts
            n = stats.successful_validations
            stats.avg_distance_when_successful = ((n - 1) * stats.avg_distance_when_successful + actual_distance) / n
        else:
            ...
            if n == 1:
                ...
                stats.avg_distance_when_failed = ((n - 1) * stats.avg_distance_when_failed + actual_distance) / n

        # Log detailed action attempts if enabled
        if DEBUG.LOG_DISTANCE_VALIDATION:
            status = "SUCCESS" if was_successful else "FAILED"
            logger.debug(f"Action {action_name} {status}: distance={actual_distance:.2f}, "
    ...
    def get_summary_stats(self) -> Dict[str, Any]:
        ...
        recent_discrepancies = [d for d in self.discrepancies if (time.time() - d.timestamp) < 300]  # Last 5 minutes
        avg_discrepancy = sum(d.distance for d in recent_discrepancies) / len(recent_discrepancies) if recent_discrepancies else 0.0

        # Action statistics summary
        action_summary = {}
        for action_name, stats in self.action_stats.items():
            success_rate = (stats.successful_validations / stats.total_attempts * 100) if stats.total_attempts > 0 else 0
    ...
    def get_agent_specific_stats(self, agent_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_discrepancy_recent': sum(recent_distances) / len(recent_distances) if recent_distances else 0,
            'max_discrepancy': max(distances),
            'action_contexts': list(set(d.action_context for d in agent_discrepancies if d.action_context)),
            'last_discrepancy_time': max(d.timestamp for d in agent_discrepancies),
            'large_discrepancy_count': sum(1 for d in agent_discrepancies if d.distance > DEBUG.MAX_ACCEPTABLE_DISCREPANCY)
        }
    ...
    def print_summary_report(self):
        ...
        print(f"  Large Discrepancies (>{DEBUG.MAX_ACCEPTABLE_DISCREPANCY}): {pos_stats['large_discrepancies']}")
        print(f"  Maximum Discrepancy: {pos_stats['max_discrepancy_seen']} units")
        print(f"  Recent Average: {pos_stats['recent_avg_discrepancy']} units ({pos_stats['recent_count']} samples)")
        print()
...
def record_position_discrepancy(
    ...
    global_stats.record_position_discrepancy(agent_id, client_pos, server_pos, action_context)
...
def record_action_distance_attempt(
    ...
    """Record an action distance validation attempt"""
    global_stats.record_action_distance_attempt(action_name, agent_pos, target_pos, max_allowed_distance, was_successful)
