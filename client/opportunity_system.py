...
class OpportunityDetector:
    ...
    def detect_opportunities(self, agent, visible_entities: List[Dict[str, Any]],
                           terrain_data: Optional[Dict[Tuple[int, int], Any]] = None) -> List[Opportunity]:
        """
        ...
        opportunities.extend(self._detect_resource_opportunities(agent, visible_entities, terrain_data))
        ...
        opportunities.extend(self._detect_emergency_opportunities(agent, visible_entities))

        # Detect exploration opportunities
        opportunities.extend(self._detect_exploration_opportunities(agent))

        logger.debug(f"Agent {agent.id[:8]} detected {len(opportunities)} opportunities")
        return opportunities

    def _detect_resource_opportunities(self, agent, visible_entities: List[Dict[str, Any]],
                                     terrain_data: Optional[Dict[Tuple[int, int], Any]]) -> List[Opportunity]:
        ...
        if terrain_data and hasattr(agent, 'agent_map') and agent.agent_map:
            opportunities.extend(self._detect_terrain_resources(agent, terrain_data, detection_range))

        return opportunities

    def _detect_combat_opportunities(self, agent, visible_entities: List[Dict[str, Any]]) -> List[Opportunity]:
        """Detect combat opportunities"""
        opportunities = []
            ...
            if self._is_valid_combat_target(entity_type, agent):
                ...
                if distance <= detection_range:
                    opportunity = Opportunity(
                        ...
                        data={"enemy_type": entity_type, "enemy_health": entity.get("health", 100)}
                    )
                    opportunities.append(opportunity)

        return opportunities

    def _detect_trade_opportunities(self, agent, visible_entities: List[Dict[str, Any]]) -> List[Opportunity]:
            ...
            if entity.get("agent_type") and entity["id"] != agent.id:
                ...
                if distance <= detection_range:
                    opportunity = Opportunity(
                        ...
                        data={"trader_type": entity.get("agent_type"), "trader_id": entity["id"]}
                    )
                    opportunities.append(opportunity)

        return opportunities

    def _detect_social_opportunities(self, agent, visible_entities: List[Dict[str, Any]]) -> List[Opportunity]:
                ...
                if distance <= detection_range:
                    ...
                    if needs_help or self._should_cooperate_with(entity, agent):
                        opportunity = Opportunity(
                            ...
                            data={"needs_help": needs_help, "ally_type": entity.get("agent_type")}
                        )
                        opportunities.append(opportunity)

        return opportunities

    def _detect_emergency_opportunities(self, agent, visible_entities: List[Dict[str, Any]]) -> List[Opportunity]:
        ...
        nearby_enemies = [e for e in visible_entities
                         if self._is_valid_combat_target(e.get("agent_type", e.get("type", "")), agent)
        ...
        if len(nearby_enemies) >= 2:
            ...
            opportunity = Opportunity(
                ...
                duration=3.0,
                required_distance=1.0,
                data={"emergency_type": "surrounded", "enemy_count": len(nearby_enemies)}
            )
            opportunities.append(opportunity)
    ...
    def _calculate_combat_value(self, entity: Dict[str, Any], agent) -> float:
        ...
        base_value = agent.personality.combat if hasattr(agent.personality, 'combat') else 3.0
    ...
    def _calculate_trade_value(self, entity: Dict[str, Any], agent) -> float:
        ...
        base_value = agent.personality.social if hasattr(agent.personality, 'social') else 2.0
    ...
    def _calculate_social_value(self, entity: Dict[str, Any], agent) -> float:
        ...
        base_value = agent.personality.cooperativeness if hasattr(agent.personality, 'cooperativeness') else 2.0
    ...
    def _is_valid_combat_target(self, entity_type: str, agent) -> bool:
        ...
        if hasattr(agent.personality, 'cooperativeness') and agent.personality.cooperativeness > 7.0:
            return entity_type == "enemy"
    ...
    def _find_safe_position(self, agent, visible_entities: List[Dict[str, Any]]) -> Tuple[float, float]:
        ...
        for entity in visible_entities:
            if self._is_valid_combat_target(entity.get("agent_type", entity.get("type", "")), agent):
                distance = math.sqrt((entity["x"] - agent.x) ** 2 + (entity["y"] - agent.y) ** 2)
    ...
    def _find_escape_position(self, agent, enemies: List[Dict[str, Any]]) -> Tuple[float, float]:
    ...
    def _detect_terrain_resources(self, agent, terrain_data: Dict[Tuple[int, int], Any],
                                detection_range: float) -> List[Opportunity]:
    ...
    def _calculate_terrain_value(self, tile_type, agent) -> float:
        ...
        if hasattr(agent, 'personality'):
            try:
                from world.tiles import TileType
                if tile_type == TileType.WOOD and hasattr(agent.personality, 'foraging'):
                    base_value *= (agent.personality.foraging / 5.0)
                elif tile_type == TileType.WATER and hasattr(agent.personality, 'fishing'):
                    base_value *= (agent.personality.fishing / 5.0)
            except ImportError:
                pass
...
class OpportunityEvaluator:
    ...
    def evaluate_opportunities(self, agent, opportunities: List[Opportunity]) -> List[Tuple[Opportunity, float]]:
        """
        ...
        logger.debug(f"Agent {agent.id[:8]} evaluated {len(scored_opportunities)} opportunities")
    ...
    def _calculate_utility(self, agent, opportunity: Opportunity) -> float:
        ...
        utility = (base_utility * distance_factor * urgency_factor *
                  personality_factor * needs_factor * context_factor)

        logger.debug(f"Opportunity {opportunity.opportunity_id} utility: {utility:.2f} "
    ...
    def _get_needs_factor(self, agent, opportunity: Opportunity) -> float:
        ...
        if opportunity.opportunity_type == OpportunityType.COMBAT and agent.health < 40.0:
    ...
    def _get_context_factor(self, agent, opportunity: Opportunity) -> float:
        ...
        if hasattr(agent, 'visible_entities'):
            nearby_enemies = sum(1 for entity in agent.visible_entities
                               ...
                               math.sqrt((entity["x"] - agent.x) ** 2 + (entity["y"] - agent.y) ** 2) < 8.0)
            if nearby_enemies > 0:
...
class OpportunitySystem:
    ...
    def update(self, agent, visible_entities: List[Dict[str, Any]],
              terrain_data: Optional[Dict[Tuple[int, int], Any]] = None) -> List[Tuple[Opportunity, float]]:
        """
    ...
    def get_best_opportunity(self, agent, visible_entities: List[Dict[str, Any]],
                           terrain_data: Optional[Dict[Tuple[int, int], Any]] = None) -> Optional[Tuple[Opportunity, float]]:
    ...
    def get_opportunities_by_type(self, opportunity_type: OpportunityType) -> List[Opportunity]:
        ...
        return [opp for opp in self.current_opportunities if opp.opportunity_type == opportunity_type]
