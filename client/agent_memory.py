...
class Memory:
    ...
    def get_strength(self, current_time: Optional[float] = None) -> float:
        ...
        decay_factor = math.exp(
            -self.decay_rate * time_since_access / 3600.0
        )  # Decay per hour
        ...
        final_strength = (
            (base_strength + reinforcement_bonus + access_bonus)
            * decay_factor
            * confidence_modifier
        )
        ...
        for dx in range(-grid_radius, grid_radius + 1):
            for dy in range(-grid_radius, grid_radius + 1):
                ...
                for memory_id in self.location_grid.get(grid_coords, []):
                    ...
                    # Check distance
                    distance = math.sqrt(
                        (memory.location[0] - x) ** 2 + (memory.location[1] - y) ** 2
                    )
                    if distance > radius:
                        continue
...
class SocialMemory:
    ...
    def add_interaction_memory(
        ...
        memory = Memory(
            ...
            if outcome in ["hostile", "very_positive"]
            else MemoryRelevance.MEDIUM,
        )
    ...
    def _update_relationship_score(
        self, agent_id: str, interaction_type: str, outcome: str
    ):
        ...
        self.relationship_scores[agent_id] = max(
            -1.0, min(1.0, self.relationship_scores[agent_id] + score_change)
        )

        # Update trust based on relationship
        if (
            self.relationship_scores[agent_id] > 0.2
        ):  # Lower threshold for trust increase
    ...
    def get_agent_memories(
        ...
        if memory_type:
            memories = [
                m for m in memories if m.content.get("interaction_type") == memory_type
            ]
...
class AgentMemory:
    ...
    def __init__(
        self, agent_id: str, max_memories: int = 1000, cleanup_interval: float = 300.0
    ):  # 5 minutes
    ...
    def remember_resource_location(
        self, x: float, y: float, resource_type: str, quality: float, quantity: int
    ) -> Memory:
    ...
    def remember_danger_zone(
        ...
        memory = Memory(
            ...
            decay_rate=0.02
            if severity > 0.8
            else 0.08,  # Severe dangers remembered longer
        )
    ...
    def remember_trade_result(
        self,
        other_agent_id: str,
        items_given: List[Dict],
        ...
        details = {
            ...
            "trade_value_received": sum(
                item.get("value", 1) for item in items_received
            ),
        }
    ...
    def get_known_resources(
        ...
        if resource_type:
            memories = [
                m for m in memories if m.content.get("resource_type") == resource_type
            ]

        return memories

    def get_danger_zones(
        self, x: float, y: float, radius: float = 15.0
    ) -> List[Memory]:
    ...
    def calculate_location_utility_modifier(
        self, x: float, y: float, action_type: str
    ) -> float:
        ...
        if is_dangerous:
            if action_type in ["explore", "gather_resources"]:
                modifier *= (
                    1.0 - danger_level * 0.8
                )  # Reduce utility for dangerous actions
            elif action_type == "flee":
                modifier *= (
                    1.0 + danger_level * 0.5
                )  # Increase flee utility in dangerous areas
    ...
    def calculate_social_utility_modifier(
        self, target_agent_id: str, action_type: str
    ) -> float:
    ...
    def periodic_cleanup(self):
        ...
        total_memories = (
            len(self.location_memory.memories)
            + sum(
                len(memories) for memories in self.social_memory.agent_memories.values()
            )
        ...
        self._update_stats()
        logger.debug(
            f"Agent {self.agent_id} memory cleanup: {total_memories} memories remaining"
        )
    ...
    def _aggressive_cleanup(self):
        ...
        all_memories.sort(
            key=lambda x: (x[1].relevance.value, x[1].get_strength()), reverse=True
        )
        ...
        for memory_type, memory in memories_to_remove:
            if memory_type == "location":
                if memory.location:
                    grid_coords = self.location_memory._get_grid_coords(
                        *memory.location
                    )
                    if (
                        memory.memory_id
                        in self.location_memory.location_grid[grid_coords]
                    ):
                        self.location_memory.location_grid[grid_coords].remove(
                            memory.memory_id
                        )
                ...
                if (
                    agent_id
                    and memory.memory_id in self.social_memory.agent_memories[agent_id]
                ):
    ...
    def _update_stats(self):
        ...
        self.memory_stats["average_strength"] = (
        ...
        return {
            ...
            "location_memory_summary": {
                ...
                "danger_zones": len(
                    [
                        m
                        for m in self.location_memory.memories.values()
                        if m.memory_type == MemoryType.DANGER_ZONE
                    ]
                ),
                "resource_locations": len(
                    [
                        m
                        for m in self.location_memory.memories.values()
