...
class TargetManager:
    ...
    def update_target_selection(
        ...
        if self.current_target:
            current_entity = self._find_entity_by_id(valid_targets, self.current_target.entity_id)
            if current_entity:
                self.current_target.update(current_entity)
            else:
                # Current target no longer visible
                logger.debug(f"Agent {self.agent_id[:8]} lost sight of target {self.current_target.entity_id[:8]}")
                self.current_target = None

        # Check if we should stick with current target (target locking)
        if (self.current_target and
            not force_reselect and
            self.current_target.get_age() < self.lock_duration and
            not self.current_target.is_stale()):

            # Stick with current target
            current_entity = self._find_entity_by_id(valid_targets, self.current_target.entity_id)
            if current_entity:
                logger.debug(f"Agent {self.agent_id[:8]} maintaining target lock on {self.current_target.entity_id[:8]} (age: {self.current_target.get_age():.1f}s)")
        ...
        if best_target:
            ...
            if self.current_target and not force_reselect:
                current_entity = self._find_entity_by_id(valid_targets, self.current_target.entity_id)
                if current_entity and not self._is_significantly_better(agent, best_target, current_entity):
                    # New target not significantly better, keep current
                    logger.debug(f"Agent {self.agent_id[:8]} keeping current target {self.current_target.entity_id[:8]} (new target not significantly better)")
                    return current_entity

            # Switch to new target
            old_target_id = self.current_target.entity_id[:8] if self.current_target else "None"
            self._set_new_target(best_target)
            logger.info(f"Agent {self.agent_id[:8]} switched target: {old_target_id} -> {self.current_target.entity_id[:8]}")
            return best_target
    ...
    def clear_target(self):
        ...
        if self.current_target:
            logger.debug(f"Agent {self.agent_id[:8]} cleared target {self.current_target.entity_id[:8]}")
            self.current_target = None
    ...
    def is_target_locked(self) -> bool:
        ...
        return (self.current_target and
                self.current_target.get_age() < self.lock_duration and
                not self.current_target.is_stale())
    ...
    def _select_best_target(self, agent, targets: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    def _select_nearest_target(self, agent, targets: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    def _select_weakest_target(self, targets: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    def _select_lowest_health_target(self, targets: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    def _is_significantly_better(self, agent, new_target: Dict[str, Any], current_target: Dict[str, Any]) -> bool:
    ...
    def _set_new_target(self, target_entity: Dict[str, Any]):
        ...
        if len(self.target_history) > self.max_history_size:
            oldest_key = min(self.target_history.keys(),
                           key=lambda k: self.target_history[k].selection_time)
            del self.target_history[oldest_key]

    def _find_entity_by_id(self, entities: List[Dict[str, Any]], entity_id: str) -> Optional[Dict[str, Any]]:
